...
class SpanOffsetOverlapError(ProtocolError):
    def __init__(self, offsets, has_overlap = False):
        self.offsets = offsets
        self.has_overlap = has_overlap

    def __str__(self):
        if self.has_overlap:
            return u'Annotation overlaps existing ones'
        if len(self.offsets) == 1:
            return u'An annotation for \"%s\" already exists. Edit the existing annotation by double-clicking on it' % (str(self.offsets[0]))
        elif len(self.offsets) == 2:
            return u'An annotation from \"%s\" to \"%s\" already exists. Edit the existing annotation by double-clicking on it' % (self.offsets[0], self.offsets[1])
        else:
            return u'The offsets [%s] overlap' % (', '.join(unicode(e) for e in self.offsets, ))
...
def open_textfile(filename, mode='rU', enc='utf8'):
    # enforce universal newline support ('U') in read modes
    if len(mode) != 0 and mode[0] == 'r' and 'U' not in mode:
        mode = mode + 'U'
    return codecs_open(filename, mode, encoding=enc, errors='strict')
...
class AllOffsets(set):
    def __init__(self, *args, **kwargs):
        set.__init__(self, *args, **kwargs)
        from collections import defaultdict
        self.occupied = defaultdict(bool)
...
class Annotations(object):
    ...
    def add_annotation(self, ann, read=False):
        ...
        try:
            ...
            offf = str(ann).split()[2:4]
            self._ann_by_offset['_'.join(str(s) for s in offf)] = ann
            if offf[0][0] != 'A':
                self._ann_offsets.add((int(offf[0]), int(offf[1])))
    ...
    def _atomic_del_annotation(self, ann):
        ...
        try:
            ...
            offf = str(ann).split()[2:4]
            del self._ann_by_offset['_'.join(str(s) for s in offf)]
            if offf[0][0] != 'A':
                self._ann_offsets.remove((int(offf[0]), int(offf[0])))
        except AttributeError:
    ...
    def get_ann_by_offset(self, offset):
        try:
            return self._ann_by_offset[offset]
        except KeyError:
            raise AnnotationNotFoundError(offset)

    def ann_exists_in_offset(self, offset):
        return offset in self._ann_by_offset

    def other_ann_exists_in_offset(self, offset):
        return not self._ann_offsets.fits(offset)
