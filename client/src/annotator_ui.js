...
//                        true, true);
      ...
      var haveChoices = function(originType, targetType) {
        var haveArcs = false;
        ...
        if (arcDragOrigin) {
          ...
          if ((id = target.attr('data-span-id')) && origin != id/* && targetValid*/) {
            ...
            var targetSpan = data.spans[id];
          	if (!haveChoices(originSpan.type, targetSpan.type)){
              temp = targetSpan;
              targetSpan = originSpan;
              originSpan = temp;
          	}
...
*/
      ...
      var showDataForm = function(dform) {
      	var ttdoc = doc;
      	if (ttdoc)
          dispatcher.post('disableFileBrowser');
       	dispatcher.post('setCollection', [coll]);
      	dispatcher.post('hideForm');
      	if (ttdoc)
          dispatcher.post(800, 'showForm', [dform]);
        else
          dispatcher.post('showForm', [dform]);
      	dform.find('input, textarea').val('');
      };
      ...
      $('#import_button').click(function() {
      	showDataForm(importForm);
      });


      var ctextfile = null;
      var cannfile = null;
      var readTxtFile = function (evt) {
        var f = evt.target.files[0]; 
        var theext = f.name.split('.').pop();
        if (theext !== "txt" || !f) {
        	$('#theFile').val('');
	      alert(f.name + " is not a valid txt file.");
        } else {
      ...
      document.getElementById('theFile').addEventListener('change', readTxtFile, false);
      var readAnnFile = function (evt) {
        var f = evt.target.files[0]; 
        var theext = f.name.split('.').pop();
        if (theext !== "ann" || !f) {
        	$('#theAnnFile').val('');
	      alert(f.name + " is not a valid ann file.");
        } else {
          var r = new FileReader();
          r.onload = function(e) { 
            var contents = e.target.result;
      ...
      document.getElementById('theAnnFile').addEventListener('change', readAnnFile, false);
      var dataImportForm = $('#data_import_form');
      var dataImportFormSubmit = function(evt) {
      	var txtfile = $('#theFile');
      	var annfile = $('#theAnnFile');
      	var txtfilename = $('#theFile').val();
      	var annfilename = $('#theAnnFile').val();
        //dispatcher.post('messages', [[["TXT File: " +  ctextfile, 'warning']]]);
        //dispatcher.post('messages', [[["ANN File: " +  cannfile, 'warning']]]);
        var opts = null;
        if (cannfile){
          opts = {
            action : 'importDocument',
      ...
      dataImportForm.submit(dataImportFormSubmit);
      dispatcher.post('initForm', [dataImportForm, {
          width: 500,
          open: function(evt) {
            keymap = {};
          },
        }]);
      $('#import_txt_button').click(function() {
        ctextfile = null;
        cannfile = null;
        $('#theFile').val('');
        $('#theAnnFile').val('');
      	showDataForm(dataImportForm);
      });



      var cxmifile = null;
      var readXmiFile = function (evt) {
        var f = evt.target.files[0]; 
        var theext = f.name.split('.').pop();
        if (theext !== "zip" || !f) {
        	$('#theXmiFile').val('');
	      alert(f.name + " is not a valid zip file.");
        } else {
          var r = new FileReader();
          r.onload = function(e) { 
            var contents = e.target.result;
      ...
      document.getElementById('theXmiFile').addEventListener('change', readXmiFile, false);
      var xmiImportForm = $('#xmi_import_form');
      var xmiImportFormSubmit = function(evt) {
      	var xmifile = $('#theXmiFile');
      	var xmifilename = $('#theXmiFile').val();
        //dispatcher.post('messages', [[["XMI File: " +  xmifilename, 'warning']]]);
        var opts = null;
          opts = {
            action : 'importXmi',
            collection : coll,
      ...
      xmiImportForm.submit(xmiImportFormSubmit);
      dispatcher.post('initForm', [xmiImportForm, {
          width: 500,
          open: function(evt) {
            keymap = {};
          },
        }]);
      $('#add_xmi_button').click(function() {
      	if (doc){
        cxmifile = null;
        $('#theXmiFile').val('');
        dispatcher.post('showForm', [xmiImportForm]);
      	//showDataForm(xmiImportForm);
      	}
      });
      ...
      var importFolderForm = $('#import_folder_form');
      var importFolderFormSubmit = function(evt) {
        var _docid = $('#import_folder_docid').val();
        var opts = {
          action : 'importFolder',
          collection : coll,
          docid  : _docid,
        ...
        dispatcher.post('ajax', [opts, function(response) {
          ...
          if (x) {
            ...
            dispatcher.post('hideForm');
            dispatcher.post('setCollection', [response.document]);
          }
        }]);
        return false;
      };
      
      importFolderForm.submit(importFolderFormSubmit);
      dispatcher.post('initForm', [importFolderForm, {
          width: 500,
          open: function(evt) {
            keymap = {};
          },
        }]);
      $('#import_folder_button').click(function() {
      	showDataForm(importFolderForm);
      });
      ...
      $('#delete_document_button').click(function() {
        ...
        var delOptions = {
          ...
          collection: coll,
          'document': theseldoc.val()
        };
        dispatcher.post('ajax', [delOptions, 'refreshFileBrowser']);
      });
