...
//       var attributeTypes = null;
      ...
      var disableFileBrowser = function(){
      	createorimportopen = true;
      };

      var enableFileBrowser = function(){
      	createorimportopen = false;
      };
      ...
      */
      ...
      var refreshFileBrowser = function() {
        //dispatcher.post('messages', [[['Refreshing file browser', 'comment']]]);
      	dispatcher.post('setCollection', [coll]);
      	//noFileSpecified();
      	hideForm();
      };
      ...
      var showFileBrowser = function() {
        ...
        if (currentForm){
          if (currentForm.selector == "#import_form" || currentForm.selector == "#import_folder_form"){
            return;
          }
        }
...
//         hideNoDocMessage();
        ...
        $.each(selectorData.header, function(headNo, head) {
          html.push('<th style="width: 100px;">' + head[0] + '</th>');
        });
        html.push('</tr>');
      ...
      var loadingForm = $('#loading_form');
      initForm(loadingForm, {
          width: 350,
          height: 105,
          resizable: false,
          no_ok: true,
      ...
      var annotationCompleteForm = $('#annotation_complete_form');
      var annotationCompleteFormSubmit = function(evt) {
      	var thedoc = doc;
        dispatcher.post('setCollection', [coll]);
        dispatcher.post('setDocument', [thedoc]);
      	dispatcher.post('hideForm');
      };
      annotationCompleteForm.submit(annotationCompleteFormSubmit);
      initForm(annotationCompleteForm, {
          width: 500,
          //height: 105,
          resizable: false,
          //no_ok: true,
      ...
      var autoAnnotateFormSubmit = function(evt) {
        ...
        dispatcher.post('ajax', [opts, function(response) {
          ...
          if (x)
            ...
            dispatcher.post('hideForm');
            //dispatcher.post('messages', [[['Done', 'warning']]]);
            dispatcher.post('showForm', [annotationCompleteForm]);
          }
        }]);
        dispatcher.post('hideForm');
        dispatcher.post('showForm', [loadingForm]);
        return false;
      };
      ...
      var onKeyDown = function(evt) {
        ...
        if (currentForm && (currentForm.selector == "#register_form" || currentForm.selector == "#auth_form" || currentForm.selector == "#change_pass_form") && code === $.ui.keyCode.ENTER) {
      ...
      var onNewSourceData = function(sourceData) {
        ...
        /* Add a download link for xmi */
          var $link = $('<a target="brat_search"/>').
              text("Extra files").
              attr('href',
                  'ajax.cgi?action=downloadFile&collection=' + encodeURIComponent(coll) +
                  '&document=' + encodeURIComponent(doc) + '&extension=' + encodeURIComponent("zip") +
                  // TODO: Extract the protocol version into somewhere global
                  '&protocol=' + 1);
        $link.button();
        $sourceFiles.append(' ');
        ...
        if (user == "admin"){
        /* Add a download link for owl */
        var $link = $('<a target="brat_search"/>').text("owl").
            attr('href', 'ajax.cgi?action=exportDocument&collection=' + encodeURIComponent(coll) +
                '&document=' + encodeURIComponent(doc) + '&extension=' + encodeURIComponent("owl") + '&protocol=' + 1);
        $link.button();
        $sourceFiles.append(' ');
        $sourceFiles.append($link);

        /* Add a download link for owls */
        var $link = $('<a target="brat_search"/>').text("owls").
            attr('href', 'ajax.cgi?action=exportDocument&collection=' + encodeURIComponent(coll) +
                '&document=' + encodeURIComponent(doc) + '&extension=' + encodeURIComponent("owls") + '&protocol=' + 1);
        $link.button();
        $sourceFiles.append(' ');
        $sourceFiles.append($link);

        /* Add a download link for ttl */
        var $link = $('<a target="brat_search"/>').text("ttl").
            attr('href', 'ajax.cgi?action=exportDocument&collection=' + encodeURIComponent(coll) +
                '&document=' + encodeURIComponent(doc) + '&extension=' + encodeURIComponent("ttl") + '&protocol=' + 1);
        $link.button();
        $sourceFiles.append(' ');
        $sourceFiles.append($link);

        /* Add a download link for ttls */
        var $link = $('<a target="brat_search"/>').text("ttls").
            attr('href', 'ajax.cgi?action=exportDocument&collection=' + encodeURIComponent(coll) +
                '&document=' + encodeURIComponent(doc) + '&extension=' + encodeURIComponent("ttls") + '&protocol=' + 1);
        $link.button();
        $sourceFiles.append(' ');
        $sourceFiles.append($link);
        }
...
//         if (show === visible) return false;
        ...
        dispatcher.post('ajax', [{
          ...
          function(response) {
              if (response.exception) {
                ...
                $('#register_button').val('Change password');
                dispatcher.post('setCollection', ['/']);
                dispatcher.post('user', [user]);
                if (user != "admin"){
        	      lastGoodCollection = ['/' + user + '/'];
        	      dispatcher.post('setCollection', [lastGoodCollection]);
        	    }
        	    else{
        	      lastGoodCollection = ['/'];
      ...
      $('#auth_button').click(function(evt) {
        if (user) {
          ...
          dispatcher.post('showForm', [authForm]);
          authForm.find('input, textarea').val('');
        }
      });
      ...
      initForm(registerForm, { resizable: false });
      var registerFormSubmit = function(evt) {
        dispatcher.post('hideForm');
        var _ruser = $('#register_user').val();
        var rpassword = $('#register_pass').val();
        dispatcher.post('ajax', [{
            action: 'register',
            user: _ruser,
            password: rpassword,
          ...
          function(response) {
              if (response.exception) {
                dispatcher.post('showForm', [registerForm]);
                $('#register_user').select().focus();
              } else {
                $('#register_user').val('');
                $('#register_pass').val('');
                $('.register').show();
      ...
      var changePassForm = $('#change_pass_form');
      initForm(changePassForm, { resizable: false });
      var changePassFormSubmit = function(evt) {
        //var passre = /^(?=.*[0-9])(?=.*[a-zA-Z])([a-zA-Z0-9]+)$/;
        //if (passre.test($('#change_pass_new').val()))
        dispatcher.post('hideForm');
        var opassword = $('#change_pass_old').val();
        var rpassword = $('#change_pass_new').val();
        dispatcher.post('ajax', [{
            action: 'changepass',
            user: user,
          ...
          function(response) {
              if (response.exception) {
                dispatcher.post('showForm', [changePassForm]);
                $('#change_pass_old').select().focus();
              } else {
                $('#change_pass_old').val('');
                $('#change_pass_new').val('');
                //$('.register').show();
      ...
      $('#register_button').click(function(evt) {
      	if (user){
       		dispatcher.post('showForm', [changePassForm]);
       		changePassForm.find('input, textarea').val('');
        }
      	else{
       		dispatcher.post('showForm', [registerForm]);
      ...
      changePassForm.submit(changePassFormSubmit);
...
//       };
      ...
      dispatcher.
          ...
          on('enableFileBrowser', enableFileBrowser).
          on('disableFileBrowser', disableFileBrowser).
          on('refreshFileBrowser', refreshFileBrowser).
